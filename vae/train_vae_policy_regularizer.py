...
class PolicyRegVAETrainer:
    ...
    def _sample_decoder_actions_and_likelihoods(self, input_latent, input_props, decoder_sample_size):
        ...
        n_pairs = input_latent.shape[0]


        # TODO: Sample logits for latent property pairs
        decoder_logits = self.model.state_decoder(z=input_latent, y=input_props).permute([1, 0, 2])

        output_actions = torch.zeros( [ n_pairs * decoder_sample_size , self.model.max_decode_steps] , dtype=torch.int64, device = self.device)
        output_likelyhoods = torch.zeros([ n_pairs * decoder_sample_size] , dtype=torch.float32, device = self.device)
        ...
        probabilities = torch.softmax(logits_expanded, dim= -1)

        # probabilities is (8*4) x 101 x 47
        probabilities_reshaped = probabilities.view(-1, probabilities.shape[-1])

        # Sample indices from the last dimension (decision_dim)
        sampled_actions_flat = torch.multinomial(probabilities_reshaped, num_samples=1, replacement=True)
        # sampled_indices_flat is 

        # Remove the extra dimension added by `num_samples=1` and restore the original shape [batch_size, max_seq_len]
        ...
        log_probs = self._compute_mol_log_probs_under_decoder(sampled_actions, logits_expanded)

        # log_probs is [n_pairs * decoder_sample_size]
        # sampled_actions is [n_paris * decoder_sample_size] x 101

        # Expand properties

        return log_probs, sampled_actions
        

    def compute_policy_regularizer(self, props, prop_samp_size = None, latent_sample_size = 4, decoder_sample_size = 10):
        '''
    ...
    def save_params(self, run_name=None, save_dir = None):
        ...
        params = {
            ...
            'latent_sample_size':self.latent_sample_size,
    ...
    def fit(self):
        ...
        for epoch_no in range(1, self.max_epochs + 1):
            ...
            if avg_valid_loss < best_val_loss:
                best_val_loss = epoch_no
            else:
                if epoch_no - best_val_loss > self.early_stopping:
                    # Early termination, save last model
                    print()
                    print(f'EARLY STOPPING: {best_val_loss} was best epoch.')
                    print()                    
                    return
    ...
    def run_epoch_vanilla_vae(self, phase, epoch = None):
        ...
        for bindex, (inp, tgt, prop) in enumerate(data_loader):
            ...
            regularizer_loss = self.regularizer_weight * self.compute_policy_regularizer(props= prop) # redundant zero grad?
            ...
            if total_batches % 20 == 0:
                if phase == 'train':
                    pbar.set_description(f'Epoch: {epoch} | Recon: {recon_loss} - KL {kl_loss} - Reg: {regularizer_loss} | Total {batch_loss}')
                else:
                    pbar.set_description(f'Validation Lossses | Recon: {recon_loss} - KL {kl_loss} - Reg: {regularizer_loss}| Total {batch_loss}')
            pbar.update(1)
...
if __name__ == "__main__":
        ...
        trainer =  PolicyRegVAETrainer(
                ...
                learning_rate = 1e-4,
                model_save_dir = '../models/POLICY_REG_CVAE/', 
